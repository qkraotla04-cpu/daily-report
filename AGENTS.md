...
## 외부 시스템 연동
- 주간 요약: Codex.ai 웹 UI 수동 연동(사용자가 TXT 다운로드 → Codex.ai 붙여넣기 → 결과 저장). API 키 사용 안 함.
...
- 전역 규칙(`~/.Codex/rules/*`)이 모두 적용됨 — 코딩 스타일/보안/TDD/검증 재기재 안 함
...
- 스펙 원본: `./docs/spec/` (구 `C:\Codex\6 daily-report-spec\` 에서 2026-04-25 이관)
